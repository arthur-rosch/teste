...
// src/use-cases/cases/task/update-task.spec.ts
...
(0, import_vitest.describe)("Update Task Status Use Case", () => {
  ...
  (0, import_vitest.it)("should update task status", () => __async(exports, null, function* () {
    const newStatus = "Testing";
    const updatedTask = yield sut.execute(
      mockTask.id,
      newStatus,
      mockTask.responsibleId
    );
    (0, import_vitest.expect)(updatedTask.id).toBe(mockTask.id);
    (0, import_vitest.expect)(updatedTask.status).toBe(newStatus);
  }));
  (0, import_vitest.it)("should not update task status if user does not exist", () => __async(exports, null, function* () {
    const newStatus = "Testing";
    yield (0, import_vitest.expect)(
      sut.execute(mockTask.id, newStatus, "nonexistent-user-id")
    ).rejects.toBeInstanceOf(ErrorHandler);
  }));
  (0, import_vitest.it)("should not update task status if task does not exist", () => __async(exports, null, function* () {
    const newStatus = "Testing";
    yield (0, import_vitest.expect)(
