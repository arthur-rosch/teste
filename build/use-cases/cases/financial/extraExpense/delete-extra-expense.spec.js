...
// src/use-cases/cases/financial/extraExpense/delete-extra-expense.ts
var DeleteExpenseUseCase = class {
  ...
  execute(_0) {
    return __async(this, arguments, function* ({
      ...
      const extraExpenseExist = yield this.financialRepository.getExtraExpenseById(expenseId);
      if (!extraExpenseExist) {
        throw new ErrorHandler(400, "Extra Expense Not Found, try again");
      }
      const deletedExtraExpense = yield this.financialRepository.deleteExtraExpense(expenseId);
      return deletedExtraExpense;
    });
  }
};

// src/use-cases/cases/financial/extraExpense/delete-extra-expense.spec.ts
...
(0, import_vitest.describe)("DeleteExpenseUseCase", () => {
  (0, import_vitest.it)("should delete an extra expense successfully", () => __async(exports, null, function* () {
    ...
    import_vitest.vi.spyOn(financialRepository, "getExtraExpenseById").mockResolvedValue(
      extraExpenseData
    );
    import_vitest.vi.spyOn(financialRepository, "deleteExtraExpense").mockResolvedValue(
      extraExpenseData
    );
    ...
    (0, import_vitest.expect)(financialRepository.getExtraExpenseById).toHaveBeenCalledWith(
      extraExpenseData.id
    );
    (0, import_vitest.expect)(financialRepository.deleteExtraExpense).toHaveBeenCalledWith(
      extraExpenseData.id
    );
  }));
  ...
  (0, import_vitest.it)("should throw an error if extra expense is not found", () => __async(exports, null, function* () {
    import_vitest.vi.spyOn(userRepository, "getUserById").mockResolvedValue(user);
    import_vitest.vi.spyOn(financialRepository, "getFinancial").mockResolvedValue({
      id: "financial-id-1",
      userId: user.id
    });
    import_vitest.vi.spyOn(financialRepository, "getExtraExpenseById").mockResolvedValue(null);
    yield (0, import_vitest.expect)(
      sut.execute({
        userId: user.id,
        expenseId: extraExpenseData.id,
