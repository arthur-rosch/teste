...
// src/use-cases/cases/financial/receivable/delete-receivable.ts
var DeleteReceivableUseCase = class {
  ...
  execute(_0) {
    return __async(this, arguments, function* ({
      ...
      const receivableExist = yield this.financialRepository.getReceivableById(receivableId);
      if (!receivableExist) {
        throw new ErrorHandler(400, "Receivable Not Found, try again");
      }
      const deletedReceivable = yield this.financialRepository.deleteReceivable(receivableId);
      return deletedReceivable;
    });
  }
};

// src/use-cases/cases/financial/receivable/delete-receivable.spec.ts
...
(0, import_vitest.describe)("DeleteReceivableUseCase", () => {
  (0, import_vitest.it)("should delete a receivable successfully", () => __async(exports, null, function* () {
    ...
    import_vitest.vi.spyOn(financialRepository, "getReceivableById").mockResolvedValue(
      receivable
    );
    import_vitest.vi.spyOn(financialRepository, "deleteReceivable").mockResolvedValue(
      receivable
    );
    ...
    (0, import_vitest.expect)(result).toEqual(receivable);
    ...
    (0, import_vitest.expect)(financialRepository.getReceivableById).toHaveBeenCalledWith(
      receivable.id
    );
    (0, import_vitest.expect)(financialRepository.deleteReceivable).toHaveBeenCalledWith(
      receivable.id
    );
  }));
  ...
  (0, import_vitest.it)("should throw an error if receivable is not found", () => __async(exports, null, function* () {
    import_vitest.vi.spyOn(userRepository, "getUserById").mockResolvedValue(user);
    import_vitest.vi.spyOn(financialRepository, "getFinancial").mockResolvedValue(
      financialRegistration
    );
    import_vitest.vi.spyOn(financialRepository, "getReceivableById").mockResolvedValue(null);
    yield (0, import_vitest.expect)(
      sut.execute({
        userId: user.id,
        receivableId: receivable.id,
