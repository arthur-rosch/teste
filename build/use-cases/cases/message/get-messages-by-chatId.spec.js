...
// src/use-cases/cases/message/get-messages-by-chatId.spec.ts
...
var GetMessagesByChatId = class {
  ...
  execute(data2) {
    return __async(this, null, function* () {
      const { chatId: chatId2 } = data2;
      const chatIdExists = yield this.messageRepository.getChatId({ chatId: chatId2 });
      if (!chatIdExists) {
...
var chatId = chance.string({ numeric: true });
var data = {
...
(0, import_vitest.describe)("Get Messages By ChatId Test", () => {
  (0, import_vitest.it)("should return messages by chatId successfully", () => __async(exports, null, function* () {
    import_vitest.vi.spyOn(messageRepository, "getChatId").mockResolvedValue(__spreadValues({}, data));
    import_vitest.vi.spyOn(messageRepository, "getMessagesByChatId").mockResolvedValue([
      __spreadValues({}, data)
    ]);
    const messages = yield sut.execute({ chatId });
    (0, import_vitest.expect)(messages).toBeDefined();
    (0, import_vitest.expect)(messages).toEqual([
      __spreadValues({}, data)
    ]);
  }));
  (0, import_vitest.it)("should return error, chatId not exists", () => __async(exports, null, function* () {
    import_vitest.vi.spyOn(messageRepository, "getChatId").mockResolvedValue(null);
    yield (0, import_vitest.expect)(sut.execute({ chatId })).rejects.toThrow(
      new ErrorHandler(400, "chatId not exists")
    );
  }));
  (0, import_vitest.it)("should return empty, chatId exists but not have messages", () => __async(exports, null, function* () {
    import_vitest.vi.spyOn(messageRepository, "getChatId").mockResolvedValue(__spreadValues({}, data));
    import_vitest.vi.spyOn(messageRepository, "getMessagesByChatId").mockResolvedValue([]);
    const messages = yield sut.execute({ chatId });
    (0, import_vitest.expect)(messages).toBeDefined();
